...
`terraform-provider-squaredup`.  Substitute
...
1. Create the schema by running the following command:
    ...
    ```text
    warning: resource squaredup_something not found in provider map; skipping
    warning: resource squaredup_something_else not found in provider map; skipping
    warning: data source squaredup_something not found in provider map; skipping
    warning: data source squaredup_something_else not found in provider map; skipping
    ```
...
1. **Add resource mappings:** For each resource in the provider, add an entry in
   the `Resources` property of the `tfbridge.ProviderInfo`, e.g.:
    ...
    "squaredup_something":      {Tok: tfbridge.MakeResource(mainPkg, mainMod, "Something")},
    "squaredup_something_else": {Tok: tfbridge.MakeResource(mainPkg, mainMod, "SomethingElse")},
    ```
...
1. **Add data source mappings:** For each data source in the provider, add an entry in the `DataSources` property of the `tfbridge.ProviderInfo`, e.g.:
    ...
    "squaredup_something":      {Tok: tfbridge.MakeDataSource(mainPkg, mainMod, "getSomething")},
    "squaredup_something_else": {Tok: tfbridge.MakeDataSource(mainPkg, mainMod, "getSomethingElse")},
    ```
...
1. **Add provider configuration overrides (not typically needed):** Pulumi's
   ...
   `squaredup_API_KEY`.  Examples of common
   uses cases:
...
1. Create a new Pulumi program in the `examples/` directory, e.g.:
    ...
    npm install
    yarn link @squaredup/pulumi-squaredup
    ```
